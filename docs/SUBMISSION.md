...
## Products, Features, Tools, and APIs Used (paste into "Tools" field)
...
## Further Improvements (paste into "Improvements" field)
...
## Demo Video Shot List (5-min script — record yourself fast)

1. **0:00-0:30** — Title card: "Onco Global Post-Visit Log Agent". Briefly describe pain point: "22 field reps. 60 mins/day on admin. We automate it."
2. **0:30-1:30** — Open Salesforce Mobile / Agent Preview. Type/dictate visit notes. Show the structured JSON returned by the prompt template. Click "Save" — Visit Log created. Show the record in Health Cloud with all fields populated.
3. **1:30-2:30** — "Now follow-up tasks." Agent extracts tasks from the same notes. Confirm each. Show created Task records linked to the Visit Log.
4. **2:30-3:30** — "Expenses. WhatsApp arrives." Show Mobile screen: agent receives a receipt photo (use `samples/receipt_ola_cab.png`). 3-second processing. Show ₹379.06 Travel Expense record auto-created with merchant "Ola", line items in Notes.
5. **3:30-4:15** — "Pre-visit prep." Ask agent "Tell me about Dr. Mehta before tomorrow." Agent shows last visit, 90-day referral trend, open tasks, then 3 personalised talking points.
6. **4:15-5:00** — Wrap: "10 deployed components. 100% Salesforce-native orchestration. Free-tier OCR for the demo, replaceable with sfdc-native multimodal once API matures." Show the org-list of deployed metadata.
...
- [ ] **Tools/APIs**: paste section 2 above
- [ ] **Improvements**: paste section 3 above
- [ ] **GitHub URL**: push this repo and paste the URL
- [ ] **Demo Video**: record using the shot list, upload (< 5 min, no AI-gen tools)
- [ ] **Admin login**: provide `awtOrg` username (`szamb@gmail.com`) + temp password to judges
- [ ] **Confirm**: original work, owned by you, no IP infringement
